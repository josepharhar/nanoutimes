...
static void utimesSync(
  ...
  if (!args[1]->IsNumber()) {
    // TODO
    args.GetReturnValue().Set(NEW_STRING("atimeS must be a number"));
    return;
  }
  uint64_t atimeS = (uint64_t)(v8::Local<v8::Number>::Cast(args[1])->Value());

  if (!args[2]->IsNumber()) {
    // TODO
    args.GetReturnValue().Set(NEW_STRING("atimeNs must be a number"));
    return;
  }
  uint64_t atimeNs = (uint64_t)(v8::Local<v8::Number>::Cast(args[2])->Value());

  if (!args[3]->IsNumber()) {
    // TODO
    args.GetReturnValue().Set(NEW_STRING("mtimeS must be a number"));
    return;
  }
  uint64_t mtimeS = (uint64_t)(v8::Local<v8::Number>::Cast(args[3])->Value());

  if (!args[4]->IsNumber()) {
    // TODO
    args.GetReturnValue().Set(NEW_STRING("mtimeNs must be a number"));
    return;
  }
  uint64_t mtimeNs = (uint64_t)(v8::Local<v8::Number>::Cast(args[4])->Value());
